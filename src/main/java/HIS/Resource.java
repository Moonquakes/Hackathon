...
public class Resource{
    ...
    public void start() throws UnsupportedEncodingException {
        ...
        if(id.length()==18&&telephone.length()==11&&password.length()==8) {
            ...
            HIS.Encrypt encrypt = new HIS.Encrypt(id,password);
            HIS.PseudoRandomGenerator psg = new HIS.PseudoRandomGenerator(encrypt.long_key);
            String[] allCipherMessage = psg.ENC(allMessage);

            HIS.Database.Processing.register(allCipherMessage[0],allCipherMessage[1],allCipherMessage[2],allCipherMessage[3],allCipherMessage[4],"无",allCipherMessage[6]);
             */
        }else{
